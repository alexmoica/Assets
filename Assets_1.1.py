...
class Assets(tk.Tk):
	...
	def __init__(self, *args, **kwargs):
		...
		tk.Tk.wm_title(self, "Assets 1.1")
...
class HomePage(tk.Frame):
	...
	def __init__(self, master, controller):			
		...
		cBtn = tk.Button(self, text="$"+str(cBalance), fg='green', width=10, command=lambda: controller.show_frame(CPage))
		...
		ccBtn = tk.Button(self, text="$"+str(ccBalance), fg=balCol, width=10, command=lambda: controller.show_frame(CCPage))
		...
		sBtn = tk.Button(self, text="$"+str(sBalance), fg='green', width=10, command=lambda: controller.show_frame(SPage))
				...
				if isFloat == True:
					if float(entry_text.get()) > 0.0:
						...
						if (accVar.get()) == "CHEQUING":
							...
							addToFile(("C"+" +"+str(entry_text.get())), note_entry.get())
						...
						if (accVar.get()) == "CREDIT":
							if ccBalance + float(entry_text.get()) > 1000.00:
								...
								addToFile(("CC"+" +"+str(entry_text.get())), note_entry.get())
								
						if (accVar.get()) == "SAVINGS":
							sBalance = sBalance + float(entry_text.get())
							sBtn['text'] = "$"+str("%.2f" % sBalance)
							addToFile(("S"+" +"+str(entry_text.get())), note_entry.get())
...
class CPage(tk.Frame):
	...
	def __init__(self, master, controller):
		...
		cLbl = tk.Label(self, text="Chequing", font=LARGE_FONT)
...
class CCPage(tk.Frame):
	...
	def __init__(self, master, controller):
		...
		ccLbl = tk.Label(self, text="Credit", font=LARGE_FONT)
...
class SPage(tk.Frame):
	...
	def __init__(self, master, controller):
		...
		sLbl = tk.Label(self, text="Savings", font=LARGE_FONT)
